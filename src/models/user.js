    ...
    *information(action, { put }) {
      ...
      if (!action.values.upload || action.values.upload.length == 0) {
        ...
        if (action.values.username) {
          ...
          let response = yield axios.post(`http://10.36.140.11:8080/api/user/amend`, JSON.stringify(obj));
          let res = yield response.data;
          let list = JSON.stringify(Object.assign({}, loginMethod, {
        ...
        if (!action.values.username) {
          ...
          let response = yield axios.post(`http://10.36.140.11:8080/api/user/updateavatar`,
            formData
          );
          let res = yield response.data;
          ...
          formData.append("id", loginMethod._id);
          formData.append("avatar", action.values.upload[0].originFileObj);
          let portrait = yield axios.post(`http://10.36.140.11:8080/api/user/updateavatar`,
            formData
          );
          // 头像
